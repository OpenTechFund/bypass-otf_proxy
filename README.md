...
- log_stats.py: An application to report on logfiles generated by [EOTK](https://github.com/alecmuffett/eotk) (the onion proxy), Nginx, Cloudfront,Fastly and Azure log files.
...
To get a list of one domain and it's mirrors use:
`python automation.py --domain=domain.com`

To test that domain in addition, use:
`python automation.py --domain=domain.com --test`
...
This is designed for cron use in a regular fashion. To probe all [OONI](https://ooni.org/) data from domains in the repository:
...
Make sure to add the Azure profile name to the domain. At the moment, that can be done only from the [web interface](bcapp/flaskapp/docs/administrator.md). 
## Analyzing Logs
